...
class PoliceAPIService:
    ...
    def _process_data_in_memory(
        ...
        logger.info(f"Processed force {force}: {len(valid_objects)} new valid rows, {len(failed_rows)} failed rows")
    ...
    def _create_stop_search_object(
        ...
        return StopSearch(
            ...
            latitude=schema_item.location.latitude if schema_item.location else None,
            longitude=schema_item.location.longitude if schema_item.location else None,
            street_id=schema_item.location.street.id
            if schema_item.location and schema_item.location.street
            else None,
