...
# --- Task Tests ---
    ...
    def test_fetch_data_for_force_exception_retry(mock_db_session):
        ...
        with patch("app.tasks.populate_stop_searches.SessionLocal", return_value=mock_db_session):
            with patch("app.tasks.populate_stop_searches.PoliceAPIService") as MockService:
                MockService.return_value.get_available_dates.side_effect = Exception("API Error")
...
def test_fetch_data_for_force_max_retries(mock_db_session):
    ...
    with patch("app.tasks.populate_stop_searches.SessionLocal", return_value=mock_db_session):
        with patch("app.tasks.populate_stop_searches.PoliceAPIService") as MockService:
            ...
            MockService.return_value.get_stop_searches.side_effect = Exception("API Error")
...
def test_insert_data_task_exception(mock_db_session):
    ...
    with patch("app.tasks.populate_stop_searches.SessionLocal", side_effect=Exception("DB Error")):
        with pytest.raises(Exception, match="DB Error"):
            func = insert_data_task.__wrapped__
            if hasattr(func, "__wrapped__"):
                func = func.__wrapped__
...
def test_importer_chunk_processing(mock_db_session):
    """Test DataImporter processing chunks."""
    importer = DataImporter(mock_db_session)
    importer._process_chunk = MagicMock()
    
    # Create a CSV with enough rows to trigger chunk processing
    chunk_size = 2
    rows = [
        {"col": "val1"},
        {"col": "val2"},
    ...
    # Mock csv.DictReader
    with patch("csv.DictReader", return_value=rows):
        with patch("builtins.open", mock_open()):
            importer.process_file("dummy.csv", chunk_size=chunk_size)
            
    # Should be called twice: once for first 2 rows, once for the last row
    assert importer._process_chunk.call_count == 2

# --- DB Session Tests ---

def test_db_session_env_vars():
    """Test DB session configuration with missing DATABASE_URL."""
    import importlib
    ...
    try:
        ...
        expected_url = "postgresql://test_user:test_pass@test_server:5432/test_db"
        assert app.db.session.DATABASE_URL == expected_url
