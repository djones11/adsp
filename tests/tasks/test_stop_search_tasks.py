...
def test_insert_rows_logs_info_when_no_csv_paths_provided(mock_logger):
    mock_db = MagicMock()
    insert_rows(mock_db, [], [], "table")
    mock_logger.info.assert_called_with("No CSV paths to process for table.")

def test_insert_rows_logs_error_when_csv_merge_fails(mock_logger):
    mock_db = MagicMock()
    with patch("app.tasks.stop_search_tasks.CSVHandler.merge_csvs"), \
...
def test_fetch_stop_search_task_returns_paths_on_success(mock_db_session, mock_service, mock_celery_self):
...
def test_fetch_stop_search_task_returns_none_when_max_retries_exceeded(mock_db_session, mock_service, mock_celery_self):
    mock_service.download_stop_search_data = AsyncMock(side_effect=Exception("API Error"))
...
def test_insert_data_task_merges_and_inserts_data_successfully(mock_db_session, mock_service, mock_csv_handler, mock_celery_self):
...
def test_fetch_stop_search_task_returns_none_when_no_dates_available(mock_db_session, mock_service, mock_celery_self):
...
def test_fetch_stop_search_task_returns_none_when_no_new_data_found(mock_db_session, mock_service, mock_celery_self):
    mock_service.download_stop_search_data = AsyncMock(return_value=None)
...
def test_ingest_stop_searches_orchestrates_tasks_with_chord_and_group(mock_group, mock_chord):
    ingest_stop_searches()
...
@patch("app.tasks.stop_search_tasks.chord")
@patch("app.tasks.stop_search_tasks.group")
def test_ingest_stop_searches_handles_setup_exceptions_gracefully(mock_group, mock_chord):
    mock_group.side_effect = Exception("Setup Error")
    ingest_stop_searches()
    # Should not raise

def test_insert_data_task_handles_db_exceptions_gracefully(mock_db_session, mock_service, mock_csv_handler, mock_celery_self):
...
def test_fetch_stop_search_task_retries_on_api_exception(mock_db_session, mock_service, mock_celery_self):
    mock_service.download_stop_search_data = AsyncMock(side_effect=Exception("API Error"))
...
def test_fetch_stop_search_task_handles_max_retries_exceeded_error(mock_db_session, mock_service, mock_celery_self):
    mock_service.download_stop_search_data = AsyncMock(side_effect=Exception("API Error"))
...
def test_insert_data_task_logs_info_when_merged_file_has_no_data_rows(mock_db_session, mock_service, mock_logger, mock_celery_self):
    ...
    with patch("builtins.open", MagicMock()) as mock_open, \
        ...
        mock_logger.info.assert_any_call(
            "No rows found in merged file for stop_searches. Input files might have been empty or missing."
        )

def test_insert_data_task_raises_exception_on_session_creation_failure(mock_celery_self):
    with patch("app.tasks.stop_search_tasks.SessionLocal", side_effect=Exception("DB Error")):
        with pytest.raises(Exception, match="DB Error"):
